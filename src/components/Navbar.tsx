      ...
      {open && (
        <div className="fixed inset-0 z-[60] bg-[#4A1420] flex flex-col items-center justify-center gap-6 animate-[fadeUp_0.4s_ease] overflow-y-auto py-16">
          ...
          <button
            ...
            className="absolute z-10 top-7 right-8 text-[#F4F1E8] text-3xl font-light leading-none"
            aria-label="Close menu"
          >
            &times;
          </button>
          <Monogram className="relative z-10 w-10 h-12 text-[#C9B7AE] mb-1" />
          <p className="relative z-10 label text-[#C9B7AE] mb-2">Prose Florals</p>
          {allLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              onClick={() => setOpen(false)}
              className="relative z-10 font-magnolia text-[#F4F1E8] text-3xl md:text-4xl tracking-[0.06em] hover:text-[#C9B7AE] transition-colors"
            >
              {link.label}
            </Link>
          ))}
          <a
            ...
            rel="noopener noreferrer"
            onClick={() => setOpen(false)}
            className="relative z-10 font-times-italic italic text-[#C9B7AE] mt-3 text-xl"
          >
            Inquire
          </a>

          <div className="relative z-10 w-20 h-20 mt-4 overflow-hidden opacity-80">
