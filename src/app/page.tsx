...
export default function Home() {
  return (
    <main className="bg-[#F4F1E8] overflow-x-clip">
      ...
      {/* ─── TEAM BAND — photo, one solid color block, one quiet
          watermark. No stacked ornaments. ─── */}
      ...
      {/* ─── TESTIMONIAL — one solid near-black block, one typographic
          move (the italic quote). Matches the "Dining Out" / "Premium
          Brand Identity" reference: confident flat color, no ornament. ─── */}
      <section className="section-y px-6 bg-[#17130F]">
        <div className="max-w-3xl mx-auto text-center">
          <p className="font-manuscript-italic text-[#F4F1E8] leading-[1.5]" style={{ fontSize: "clamp(24px, 3.2vw, 38px)" }}>
            &ldquo;From start to finish I had the most wonderful experience with Prose Florals. They were <em>exactly</em> what we had envisioned, maybe <em>even</em> better!&rdquo;
          </p>
          <p className="font-times-italic italic text-[#F4F1E8]/60 text-lg mt-8">— Meaghan &amp; Abel, La Brassa</p>
        </div>
      </section>
      ...
      {/* ─── EXPLORE CTA — washed photo collage behind display type ─── */}
      <section className="relative grid grid-cols-2 md:grid-cols-4 h-[60vh] md:h-[72vh] overflow-hidden bg-[#33302A]">
        {[IMG.brideMoody, IMG.coupleWalk, IMG.brideBrick, IMG.reception].map((src, i) => (
          <div key={i} className="relative opacity-30">
            <Image src={src} alt="" fill sizes="25vw" className="object-cover grayscale" />
          </div>
        ))}
